...
import "./istio-virtualservice-chart.scss";

import { v4 as uuidv4 } from "uuid";

import { Gateway, GatewayApi } from "../../istio-objects/Gateway";

import {
  VirtualService,
  VirtualServiceApi,
  VirtualServiceHttp,
  VirtualServiceHttpRouteDestinationPort,
} from "../../istio-objects/VirtualService";
@observer
export class IstioVirtualServiceChart extends React.Component<
  Renderer.Component.KubeObjectDetailsProps<VirtualService>
> {
  ...
  async componentWillReceiveProps(
    nextProps: Renderer.Component.KubeObjectDetailsProps<VirtualService>
  ) {
  ...
  async dealChartData(
    props: Renderer.Component.KubeObjectDetailsProps<VirtualService>
  ) {
    //VirtualService ns
    const virtualServiceNs = props.object.getNs();

    const nodeDataArray: any[] = [
      {
        key: "virtualservice." + virtualServiceNs + "." + props.object.getId(),
        text:
          "VirtualService\n" +
          props.object.getNs() +
          "." +
    ...
    //搜寻对应service
    const https: VirtualServiceHttp[] = props.object.spec["http"] || [];
    const defaultPort: VirtualServiceHttpRouteDestinationPort = { number: 0 };
    for (let ii = 0, ss = https.length; ii < ss; ii++) {
      ...
      if (http.route && http.route.length > 0) {
        for (let iii = 0, sss = http.route.length; iii < sss; iii++) {
          ...
          const host = route.destination ? route.destination.host : "";

          const port = route.destination ? route.destination.port : defaultPort;

          const toServiceObjectList =
            (await Renderer.K8sApi.serviceApi.list({
              namespace: virtualServiceNs,
            })) || [];
          console.log("toServiceObjectList=", toServiceObjectList);
            ...
            if (
              ...
              linkDataArray.push({
                ...
                text:
                  host +
                  ":" +
                  (port && port.number ? port.number.toString() : "0"),
              });
            }
          });
        }
    ...
    const gatewayList: Gateway[] = await gatewayApi.list();
    gatewayList.forEach((item) => {
      if (
        (item.getNs() == "istio-system" ||
          item.getNs() == props.object.getNs()) &&
        props.object.spec["gateways"].includes(item.getName())
      ) {
        nodeDataArray.push({
          key: "gateway." + item.getNs() + "." + item.getId(),
          text: "Gateway\n" + item.getNs() + "." + item.getName(),
          color: "lightblue",
          type: "gateway",
          ns: item.getNs(),
        ...
        linkDataArray.push({
          key: uuidv4(),
          from: "gateway." + item.getNs() + "." + item.getId(),
          to:
            "virtualservice." +
            props.object.getNs() +
            "." +
          ...
          text: props.object.spec["hosts"].join(","),
        });
      }
    });
    ...
    return (
      <div>
        ...
        <ReactDiagram
          ...
          divClassName="istio-virtualservice-chart-diagram-component"
